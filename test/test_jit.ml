...
let check_state ~msg ~tol (a : M.state) (b : M.state) =
  M.State.map2
    (fun x y -> check_leaf ~msg ~tol x y)
    a
    b
  |> ignore
...
let test_parity () =
  ...
  let steps = 50 in
  let eager = run_steps ~steps ~step:(fun ~state ~grads -> M.step ~config ~state ~grads) in
  let jitted =
...
let test_state_threads () =
  ...
  is_true ~msg:"loss is finite after two jitted steps"
...
let test_no_retrace () =
  ...
  is_true
    ~msg:"ten replays cost less than the first trace"
    Float.(replayed < 5. *. first)
